from ....gps import gps  # requires using `gps` git submodule
...
class Location:
    ...
    def bearing_to(self, lat: float, lon: float):
        ...
        return (
            ...
            else (resultbearing - 360 if resultbearing > 180 else resultbearing)
        )

    # Starts updating fields from the GPS box
    def start_GPS_thread(self):
        self.running = True
        t = Thread(target=self.update_fields_loop, name=("update GPS fields"), args=())
        t.daemon = True
        t.start()
