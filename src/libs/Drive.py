...
from ..RoverMap.server import MapServer

from . import UDPOut
from . import Location
from . import ARTracker
...
class Drive:
    def __init__(self, baseSpeed, cameras):
        ...
        self.startMap(self.updateMap, 0.5)
        sleep(0.1)
    ...
    def getSpeeds(self, speed, error, time, kp=0.35, ki=0.000035):
        values = [0, 0]
    ...
    def driveAlongCoordinates(self, locations, id1, id2=-1):
        ...
        for loc in locations:
            ...
            while self.gps.distance_to(loc[0], loc[1]) > 0.0025:  # .0025km
                bearingTo = self.gps.bearing_to(loc[0], loc[1])
                print(self.gps.distance_to(loc[0], loc[1]))
                self.speeds = self.getSpeeds(
                    self.baseSpeed, bearingTo, 100
    ...
    def trackARMarker(self, id1, id2=-1):
        ...
        if id2 == -1:
            ...
            # Tracks down the tag
            while (
                self.tracker.distanceToMarker > stopDistance
                or self.tracker.distanceToMarker == -1
            ):  # -1 means we lost the tag
                _markerFound = self.tracker.findMarker(
                    id1, cameras=1
                )  # Looks for the tag
                ...
                if self.tracker.distanceToMarker > stopDistance:
                    ...
                    timesNotFound = 0
                    print(
                        f"Tag is {self.tracker.distanceToMarker}cm away at {self.tracker.angleToMarker} degrees"
                    )
            ...
            _coords = self.gps.get_coordinates(
                self.tracker.distanceToMarker / 100000.0 + 0.004,
                self.tracker.angleToMarker,
            )
