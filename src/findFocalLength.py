...
cam.set(
    cv2.CAP_PROP_FOURCC,
    cv2.VideoWriter.fourcc(format[0], format[1], format[2], format[3]),
)
...
tagDict = aruco.getPredefinedDictionary(aruco.DICT_4X4_50)

while True:
    try:
        # takes image and detects markers
        ret, image = cam.read()
        detector = aruco.ArucoDetector(tagDict)
        corners, markerIDs, rejected = detector.detectMarkers(image)

        tagWidth = 0
        if markerIDs is not None and markerIDs[0] == 4:
