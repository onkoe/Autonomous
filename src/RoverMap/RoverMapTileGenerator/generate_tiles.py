...
def generateTiles(
    image_name, northWestLatLon, southEastLatLon, baseZoomValue, maxZoomValue
):
    ...
    for zoomInFactor in range(0, maxZoomValue - baseZoomValue + 1):
        ...
        for tileDx in range(0, numTilesToCreate):
            for tileDy in range(0, numTilesToCreate):
                ...
                tileLatBounds = [
                    min(tileTopLeftLatLon[0], tileBottomRightLatLon[0]),
                    max(tileTopLeftLatLon[0], tileBottomRightLatLon[0]),
                ]
                tileLonBounds = [
                    min(tileTopLeftLatLon[1], tileBottomRightLatLon[1]),
                    max(tileTopLeftLatLon[1], tileBottomRightLatLon[1]),
                ]
                ...
                if isIntervalNonempty(latRangeToDraw) and isIntervalNonempty(
                    lonRangeToDraw
                ):
                    ...
                    yPixelRange = (
                        round(
                            256
                            * (
                                (lonRangeToDraw[0] - tileLonBounds[0])
                                / (tileLonBounds[1] - tileLonBounds[0])
                            )
                        ),
                        round(
                            256
                            * (
                                (lonRangeToDraw[1] - tileLonBounds[0])
                                / (tileLonBounds[1] - tileLonBounds[0])
                            )
                        ),
                    )
                    ...
                    xPixelRange = (
                        ...
                        - round(
                            256
                            * (latRangeToDraw[1] - tileLatBounds[0])
                            / (tileLatBounds[1] - tileLatBounds[0])
                        ),
                        256
                        - round(
                            256
                            * (latRangeToDraw[0] - tileLatBounds[0])
                            / (tileLatBounds[1] - tileLatBounds[0])
                        ),
                    )
                    ...
                    imageBottomRight = latLonToPixels(
                        latRangeToDraw[0], lonRangeToDraw[1]
                    )
                    mapSeg = mapImage[
                    ...
                    blankImage[
                        xPixelRange[0] : xPixelRange[1], yPixelRange[0] : yPixelRange[1]
                    ] = cv.resize(
                        mapSeg,
                        (
                            yPixelRange[1] - yPixelRange[0],
                            xPixelRange[1] - xPixelRange[0],
                        ),
                    )

                    cv.imwrite(
                        "tiles/z{}, x{}, y{}.jpg".format(z, tileX, tileY), blankImage
                    )
                else:
                    continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generates slippy map tiles from a very hir-res map image"
    )
    parser.add_argument("image_name", help="The large map image to take tiles from")
    parser.add_argument(
        "southLatitude",
        help="The latitude of the south edge of the map region in the image",
        type=float,
    )
    parser.add_argument(
        "northLatitude",
        help="The latitude of the north edge of the map region in the image",
        type=float,
    )
    parser.add_argument(
        "westLongitude",
        help="The longitude of the west edge of the map region in the image",
        type=float,
    )
    parser.add_argument(
        "eastLongitude",
        help="The longitude of the east edge of the map region in the image",
        type=float,
    )
    parser.add_argument(
        "-b",
        "--baseZoomValue",
        help="What zoom level the entire map represents. As far as I can tell, you can set this to any arbitrary reasonable value. Default is 10.",
        type=int,
        required=False,
        default=10,
    )
    parser.add_argument(
        "-m",
        "--maxZoomValue",
        help="The maximum zoom level of tile to be generated. Default is 19.",
        type=int,
        required=False,
        default=19,
    )
    ...
    generateTiles(
        args.image_name,
        [args.northLatitude, args.westLongitude],
        [args.southLatitude, args.eastLongitude],
        args.baseZoomValue,
        args.maxZoomValue,
    )
